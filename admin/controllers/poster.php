 ...
 //分类列表
		...
		if(!empty($data)){//增加新栏目
		  ...
		  //添加数据
			...
			if($this->db->affected_rows()>0){
			     $this->message('新增成功！',site_url($this->router->class.'/lists/'.$data['info']['type']));
			}else{
			     $this->message('新增失败 ！',site_url($this->router->class.'/lists/'.$data['info']['type']));
			}
		}else{
		   $data['type']=$this->uri->segment(3);	
   ...
   public  function edit(){
            ...
            if(!empty($data)){//修改信息
				...
				if($this->db->affected_rows()>0){
					 $this->message('修改成功！',site_url($this->router->class.'/lists/'.$data['info']['type']));
				}else{
					 $this->message('修改失败 ！',site_url($this->router->class.'/lists/'.$data['info']['type']));
				}
			}else{//显示修改信息界面
