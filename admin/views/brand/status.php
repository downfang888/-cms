...
<?php if(!empty($data['list'])){ foreach ($data['list'] as $v){?>
 ...
 <?php }}else{?>
	...
	<?php } ?> 
     ...
     <div class="btn">
    	<input type="button" class="button" value="通过审核" onclick="myform.action='<?=base_url();?>index.php/brand/status_ok';myform.submit();"/>
		<input type="button" class="button" value="删除" onclick="javascript:if(ConfirmDel()){myform.action='<?=base_url();?>index.php/brand/del';myform.submit();}"/>
	</div>
