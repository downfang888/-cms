...
<div class="cate_top">
	 ...
	 <span>|</span> <a href="<?=base_url();?>index.php/links/status"  class="cur">审核链接</a>
...
<?php if(!empty($data['list'])){ foreach ($data['list'] as $v){?>
    <tr>
		...
		<td align='center'><a href="<?=base_url();?>index.php/links/edit/<?=$v['id']?>">修改</a> | <a onclick="return ConfirmDel();"  href="<?=base_url();?>index.php/links/del/<?=$v['id']?>">删除</a></td>
	</tr>
 ...
 <?php }}else{?>
	...
	<?php } ?> 
     ...
     <div class="btn">
    	<input type="button" class="button" value="通过审核" onclick="myform.action='<?=base_url();?>index.php/links/status_ok';myform.submit();"/>
		<input type="button" class="button" value="删除" onclick="javascript:if(ConfirmDel()){myform.action='<?=base_url();?>index.php/links/del';myform.submit();}"/>
	</div>
