...
<div class="cate_top">
	 ...
	 <span>|</span> <a href="<?=base_url();?>index.php/message/status"  class="a">审核留言</a>
...
<?php if(!empty($data['list'])){ foreach ($data['list'] as $v){?>
    <tr>
		...
		<td align='center'><?php if(!empty($v['ask'])){echo '已回复';}else{echo "<span style='color:red'>待回复</span>";}?></td>
		<td align='center'><?=date('Y-m-d H:i:s',$v['uptime'])?></td>
		<td align='center'><a href="<?=base_url();?>index.php/message/edit/<?=$v['id']?>">查看并回复</a> | <a onclick="return ConfirmDel();"  href="<?=base_url();?>index.php/message/del/<?=$v['id']?>">删除</a></td>
	</tr>
 ...
 <?php }}else{?>
	...
	<?php } ?> 
     ...
     <div class="btn">
	<input type="button" class="button" value="通过审核" onclick="myform.action='<?=base_url();?>index.php/message/status_ok';myform.submit();"/>
