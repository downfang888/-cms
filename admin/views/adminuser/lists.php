...
<div class="pad-10">
<div class="cate_top">
     <a href="<?=base_url();?>index.php/adminuser/add" class="on"><em>增加管理员</em></a>
  </div>
...
<?php if(!empty($list)){ foreach ($list as $v){?>
    <tr>
		...
		<td align='center'><a href="<?=base_url();?>index.php/adminuser/edit/<?=$v['id']?>">修改</a> | <a onclick="return ConfirmDel();"  href="<?=base_url();?>index.php/adminuser/del/<?=$v['id']?>">删除</a></td>
	</tr>
 <?php }}else{?>
				<tr>
    ...
    <div class="btn">
    	<input type="button" class="button" value="排序" onclick="myform.action='<?=base_url();?>index.php/adminuser/order';myform.submit();"/>
		<input type="button" class="button" value="删除" onclick="javascript:if(ConfirmDel()){myform.action='<?=base_url();?>index.php/adminuser/del';myform.submit();}"/>
			</div>
