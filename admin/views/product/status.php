...
<form name="searchform" action="<?=base_url();?>index.php/product/search" method="get" >
...
<?php if(!empty($data['list'])){ foreach ($data['list'] as $v){?>
    <tr>
		...
		<td>
		<a href="<?=base_url().'../product/show/'.$v['id'].'.html?st=shenhe'?>" target="_blank"><?=$v['title']?></a>
		...
		<td align='center'><a href="<?=base_url();?>index.php/product/edit/<?=$v['id']?>">修改</a> | <a onclick="return ConfirmDel();"  href="<?=base_url();?>index.php/product/del/<?=$v['id']?>">删除</a></td>
	</tr>
 ...
 <?php }}else{?>
	...
	<?php } ?> 
     ...
     <div class="btn">
     <input type="button" class="button" value="通过审核" onclick="myform.action='<?=base_url();?>index.php/product/status_ok';myform.submit();"/>
		<input type="button" class="button" value="删除" onclick="javascript:if(ConfirmDel()){myform.action='<?=base_url();?>index.php/product/del';myform.submit();}"/>
	</div>
