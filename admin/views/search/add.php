...
$(function(){
	...
	$("#title").formValidator({onshow:"请输入关键词",onfocus:"关键词长度应该为2-40个字符"}).inputValidator({min:2,max:40,onerror:"关键词长度应该为2-40个字符"});
...
<div class="nav10"></div>
<form name="myform" id="myform" action="<?=base_url();?>index.php/search/add" method="post" enctype="multipart/form-data" />
<div class="pad-10">
<div class="col-tab">
...
<table width="100%" class="table_form ">
      <tr>
        <th>关键字：</th>
        <td>
       <input type="text" name="info[title]" id="title" class="input-text" size="30" value="">
		</td>
      </tr>
 <tr>
        <th>点击次数：</th>
        <td><input type="text" name="info[hits]" id="hits" class="input-text" size="20" value="0"></td>
      </tr>     
<tr>
        <th>发布时间：</th>
        <td><input type="text" name="info[uptime]" id="uptime"  size="25" value="<?=date('Y-m-d H:i:s',time())?>" class="date"></td>
      </tr>
