...
<div class="nav10"></div>
<form name="myform" id="myform" action="<?=base_url();?>index.php/category/add" method="post"  enctype="multipart/form-data" />
<div class="pad-10">
<div class="col-tab">
...
<table width="100%" class="table_form ">
        ...
        <td>
		 <select name="info[modelid]" id="modelid" ><option value='' selected>请选择模型</option>
	<?php foreach ($data['model'] as $v){ ?>
<option value="<?=$v['id']?>"  <?php if(isset($data['res']['modelid']) && $data['res']['modelid']==$v['id'] )echo 'selected';?>><?=$v['name']?></option>
<?php }?>	
</select> 
	 </td>
      </tr>
...
<?php foreach ($data['list'] as $v){
	     ...
	     $strpad=str_pad('',$m*13,'|&nbsp;&nbsp;');
?>			
		<option value='<?=$v['id']?>' <?php if($data['id']==$v['id'])echo 'selected';?>><?=$strpad?><?=$v['catname']?></option>
<?php } ?>  
</select>
		</td>
      </tr>
	...
	<tr>
		<tr>
        <th>栏目描述：</th>
        <td> <textarea name="info[content]" maxlength="255" style="width:300px;height:60px;"></textarea></td>
      </tr>
	<tr>
        <th>栏目图片：</th>
        <td>
...
<tr>
        <th>外部链接：</th>
        <td><input type="text" name="info[url]" id="url" class="input-text" value="" size="50"><span style="color:#0066FF">只针对外部链接模型有效</span></td>
      </tr>
	  <tr>
        <th>是否在导航显示：</th>
        <td><input name="info[ismenu]" value="1" checked="" type="radio"> 是&nbsp;&nbsp;&nbsp;&nbsp;
	  <input name="info[ismenu]" value="0" type="radio"> 否</td></td>
      </tr>
</table>
...
<table width="100%" class="table_form ">
	<tr>
      ...
      <td><input name='info[seotitle]' type='text' class="input-text" value='' size='60' maxlength='60'></td>
    </tr>
    <tr>
      <th ><strong>META Keywords（栏目关键词）</strong><br/>关键字中间用半角逗号隔开</th>
      <td><textarea name='info[seokeyword]' id='meta_keywords' style="width:90%;height:40px"></textarea></td>
    </tr>
    <tr>
      <th ><strong>META Description（栏目描述）</strong><br/>针对搜索引擎设置的网页描述</th>
      <td><textarea name='info[seodescription]' id='meta_description' style="width:90%;height:50px"></textarea></td>
    </tr>
</table>
</div>
