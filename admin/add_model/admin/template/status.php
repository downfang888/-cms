...
<div class="cate_top">
     <a href="<?=base_url();?>index.php/<$tablename>/" class="on"><em>内容管理</em></a>
     <span>|</span><a href="<?=base_url();?>index.php/<$tablename>/add" class="on"><em>添加内容</em></a> 
...
<?php if(!empty($data['list'])){ foreach ($data['list'] as $v){?>
    <tr>
		...
		<td>
        <a href="<?=base_url().'../<$tablename>/show/'.$v['id'].'.html?st=shenhe'?>" target="_blank"><?=$v['title']?></a>
		...
		<td align='center'><a href="<?=base_url();?>index.php/<$tablename>/edit/<?=$v['id']?>">修改</a> | <a onclick="return ConfirmDel();"  href="<?=base_url();?>index.php<$tablename>/del/<?=$v['id']?>">删除</a></td>
	</tr>
 ...
 <?php }}else{?>
	...
	<?php } ?> 
     ...
     <div class="btn">
       <input type="button" class="button" value="通过审核" onclick="myform.action='<?=base_url();?>index.php/<$tablename>/status_ok?status=1';myform.submit();"/>
	 <input type="button" class="button" value="退稿" onclick="myform.action='<?=base_url();?>index.php/<$tablename>/status_ok?status=2';myform.submit();"/>
		<input type="button" class="button" value="删除" onclick="javascript:if(ConfirmDel()){myform.action='<?=base_url();?>index.php/<$tablename>/del';myform.submit();}"/>
	</div>
